...
### 快速定位步驟 (Windows 使用者推薦)：
1. 打開您的**檔案總管**，找到您存放本專案的資料夾。
2. 確認您能看到 `package.json` 這個檔案（如果沒看到，可能是在下一層資料夾內）。
3. 點擊資料夾上方路徑列的**空白處**，將路徑選取。
4. 直接輸入 `cmd` 然後按 **Enter**。
5. 跳出的黑色視窗即已位於「根目錄」，此時輸入 `dir` 必定能看到 `package.json`。

**驗證方式**：
輸入 `ls` (Mac) 或 `dir` (Windows)，若看到 `package.json` 則代表路徑正確。
